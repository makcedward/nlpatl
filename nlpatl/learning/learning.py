...
class Learning:
    ...
    def __init__(
        self,
        sampling: Union[str, Callable] = None,
        embeddings: Union[str, Embeddings] = None,
        embeddings_type: str = "",
    ...
    def get_return_object(self, d, return_type):
        assert (
            return_type in self.RETURN_TYPES
    ...
    def init_sampling(self, sampling):
        if type(sampling) is str:
            ...
            raise ValueError(
                "`{}` does not support. Supporting str or function only".format(
                    sampling
                )
            )
    ...
    def init_embeddings(self, embeddings, embeddings_type, embeddings_model_config):
        if type(embeddings) is str:
            ...
            raise ValueError(
                "`{}` does not support. Supporting str or "
    ...
    def build_model(
        self, name_or_model, possible_models, model_config, default_model_config
    ):
        if type(name_or_model) is str:
            ...
            if name in possible_models:
                ...
                raise ValueError(
                    "`{}` does not support. Supporting {} only".format(
                        name, "`" + "`".join(possible_models.keys()) + "`"
                    )
                )
    ...
    def filter(
        self, data: Union[List[str], List[float], np.ndarray], indices: np.ndarray
    ) -> Union[List[str], List[float], np.ndarray]:
    ...
    def educate(
        ...
        if type(x_features) in [int, float, list]:
            ...
            if self.learn_x_features is not None:
                self.learn_x_features = np.concatenate(
                    (self.learn_x_features, np.array([x_features])), axis=0
                )
            else:
                self.learn_x_features = np.array([x_features])
        else:
            assert False, (
                "{} data type does not support in `x_features` yet. "
                "Only support `{}`".format(
                    type(x), "`, `".join(["int", "float", "list", "np.ndarray"])
                )
            )
