...
class MismatchFarthestLearning(Learning):
    ...
    def __init__(
        self,
        clustering_sampling: Union[str, Callable],
    ...
    def explore_first_stage(
        self, x: np.ndarray, num_sample: int = 1
    ) -> Union[Dataset, dict]:
    ...
    def explore_second_stage(self, x: np.ndarray, num_sample: int = 2):
        ...
        kmean = self.init_kmean(x=unannotated_x_features, model_config=model_config)
        ...
        classification_preds = [
            self.classification_model.label_decoder[y] for y in preds
        ]
        ...
        new_groups = np.array(
            [
                unique_y_encoder[learn_y_decoder[g]][0]
                for g in clustering_predictions.groups
            ]
        )
    ...
    def explore_educate_in_notebook(
        ...
        valuable_data = self.explore_first_stage(x_features, num_sample=num_sample)
