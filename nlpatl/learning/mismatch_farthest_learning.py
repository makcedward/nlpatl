...
class MismatchFarthestLearning(Learning):
	"""
		...
		|	7. Repeat Step 2 to 6 until acquire enough data points or reach other
			exit criteria.
		
		:param clustering_sampling: Clustering sampling method for stage 1 exploration. 
			Providing certified methods name (`kmeans`)
			Refer to nlpatl.sampling.clustering.
		:type sampling: :class:`nlpatl.sampling.clustering`
		...
		:param embeddings_model_config: Configuration for embeddings models. Optional
		...
		:param clustering_model_config: Configuration for clustering models. Optional
	...
	def __init__(self, clustering_sampling: Union[str, Sampling, Callable],
		...
		self.mismatch_sampling = MismatchSampling().sample
		self.farthest_sampling = FarthestSampling().sample
