...
class TestModelClassificationSkLearn(unittest.TestCase):
    ...
    def test_parameters(self):
        ...
        assert (
            "l2" == classification.model.penalty
        ), "Invalid when using default parameters"

        model_config = {}
        classification = SkLearnClassification(
            "logistic_regression", model_config=model_config
        )
        assert (
            "l2" == classification.model.penalty
        ...
        assert "l1" == classification.model.penalty, "Invalid when passing parameter"

        model_config = {"gamma": "auto"}
        classification = SkLearnClassification("svc", model_config=model_config)
        assert (
            "auto" == classification.model.gamma
        ), "Invalid when passing emtpy parameters"
        assert (
            type(classification.model) is sklearn.svm._classes.SVC
        ), "Unable to initialize SVM"

        classification = SkLearnClassification("linear_svc")
        assert (
            type(classification.model) is sklearn.svm._classes.LinearSVC
        ), "Unable to initialize Linear SVM"

        classification = SkLearnClassification("random_forest")
        assert (
            type(classification.model)
            is sklearn.ensemble._forest.RandomForestClassifier
        ), "Unable to initialize Random Forest"

        classification = SkLearnClassification("sgd")
        assert (
            type(classification.model)
            is sklearn.linear_model._stochastic_gradient.SGDClassifier
        ), "Unable to initialize SGD"

        classification = SkLearnClassification("knn")
        assert (
            type(classification.model)
            is sklearn.neighbors._classification.KNeighborsClassifier
        ), "Unable to initialize KNN"

        classification = SkLearnClassification("gbdt")
        assert (
            type(classification.model)
            is sklearn.ensemble._gb.GradientBoostingClassifier
