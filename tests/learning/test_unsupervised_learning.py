...
class TestLearningUnsupervised(unittest.TestCase):
	...
	def test_sklearn_extra(self):
		...
		self.learning.clustering_name, self.learning.clustering_model = self.learning.init_clustering_model(
			'kmedoids', new_clustering_model_config)
		assert type(self.learning.clustering_model.model) is \
			sklearn_extra.cluster._k_medoids.KMedoids, 'Unable to initialize KMedoids'
